...
# -------------- load the intentsa -------------------
data_file = open('intentsa.json', encoding="utf8").read()
...
# loop through the intentsa.json file
for data in intentsa['intents']:
    for pattern in data['patterns']:
    # remove stopwords with a for loop
    # after cleaining we'll add it to corpus #tokenize each statement into words
...
for cor in corpus: # tokenized words with thier tags: ['أهلا','مرحبا']greeting
    ...
    output_row[classes.index(cor[1])] = 1 # cor[1] = الرد
...
sgd=keras.optimizers.SGD(learning_rate=0.01,decay=1e-6,momentum=0.9,nesterov=True) #learning rate =0.01
